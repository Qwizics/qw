...
class TenantRepository {
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await MongooseRepository.wrapWithSessionIfExists(
      Course(options.database).deleteMany({ tenant: id }),
      options,
    );

    await MongooseRepository.wrapWithSessionIfExists(
      CourseRegistration(options.database).deleteMany({ tenant: id }),
      options,
    );

    await MongooseRepository.wrapWithSessionIfExists(
      Exam(options.database).deleteMany({ tenant: id }),
      options,
    );

    await MongooseRepository.wrapWithSessionIfExists(
      CourseLecture(options.database).deleteMany({ tenant: id }),
      options,
    );

    await MongooseRepository.wrapWithSessionIfExists(
      CoursePayment(options.database).deleteMany({ tenant: id }),
      options,
    );

    await MongooseRepository.wrapWithSessionIfExists(
      ExamQuestion(options.database).deleteMany({ tenant: id }),
      options,
    );

    await MongooseRepository.wrapWithSessionIfExists(
      ExamAnswer(options.database).deleteMany({ tenant: id }),
      options,
    );

    await MongooseRepository.wrapWithSessionIfExists(
      ExamResult(options.database).deleteMany({ tenant: id }),
      options,
    );
