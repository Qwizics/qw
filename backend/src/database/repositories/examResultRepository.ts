...
class ExamResultRepository {
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await MongooseRepository.wrapWithSessionIfExists(
      ExamResult(options.database).findById(id),
      options,
    );
    ...
    await MongooseRepository.wrapWithSessionIfExists(
      ExamResult(options.database).updateOne(
        { _id: id },
        {
          ...data,
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await MongooseRepository.wrapWithSessionIfExists(
      ExamResult(options.database).deleteOne({ _id: id }),
      options,
    );
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      ExamResult(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    let rows = await ExamResult(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort)
  ...
  static async findAllAutocomplete(search, limit, options: IRepositoryOptions) {
    ...
    const records = await ExamResult(options.database)
      .find(criteriaAnd)
      .limit(limitEscaped)
      .sort(sort);
  ...
  static async _createAuditLog(action, id, data, options: IRepositoryOptions) {
    await AuditLogRepository.log(
      {
        entityName: ExamResult(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
